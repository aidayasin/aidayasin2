...
async function getWeather(city) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=801b7575032bd6b4900529d168d397cc&units=metric`
  ).catch(handleError);
  ...
  async function getTime(data) {
    try {
      const response = await fetch(
        `https://secure.geonames.org/timezoneJSON?lat=${data.coord.lat}&lng=${data.coord.lon}&username=aidayasin`
      );
      const responseJson = await response.json();
